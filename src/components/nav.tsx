...
export function Nav() {
  ...
  return (
    <header className="fixed inset-x-0 top-0 z-50 px-3 pt-3 sm:px-5 sm:pt-4">
      <nav className="mx-auto flex max-w-[1400px] items-center justify-between gap-3 rounded-full border border-ink-900/10 bg-cream-50/85 py-2 pl-3 pr-2 shadow-card backdrop-blur-xl">
      ...
      <AnimatePresence>
        {mobileOpen && (
          <motion.div
            ...
            exit={{ opacity: 0, y: -8 }}
            className="mx-auto mt-2 flex max-w-[1400px] flex-col gap-1 rounded-3xl border border-ink-900/10 bg-cream-50 p-3 shadow-warm md:hidden"
          >
            {LINKS.map((link) => (
              <Link
                key={link.href}
